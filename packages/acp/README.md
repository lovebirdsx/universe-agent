...
ACP (Agent Client Protocol) server for UniverseAgent - enables integration with IDEs like Zed, JetBrains, and other ACP-compatible clients.

## Overview

This package wraps UniverseAgent with the [Agent Client Protocol (ACP)](https://agentclientprotocol.com), allowing your AI agents to communicate with code editors and development tools through a standardized protocol.
...
```bash
npm install @universe-agent/acp
# or
pnpm add @universe-agent/acp
```
...
# With custom options
npx universe-agent-acp --name my-agent --debug
...
npx universe-agent-acp \
  ...
  --skills ./skills,~/.universe-agent/skills \
  --debug
```
...
| ---------------------- | ----- | ------------------------------------------------- |
| `--name <name>`        | `-n`  | Agent name (default: "universe-agent")    |
...
const server = new UniverseAgentServer({
  ...
  // Server options
  serverName: "my-universe-agent-acp",
  serverVersion: "1.0.0",
  workspaceRoot: process.cwd(),
  debug: true,
});
...
| --------------- | -------------------------------------- | ----------------------------------------------- |
| `agents`        | `UniverseAgentConfig \| UniverseAgentConfig[]` | Agent configuration(s)                          |
| `serverName`    | `string`                               | Server name for ACP (default: "universe-agent-acp") |
...
│   ┌─────────────────────▼──────────────────────────────┐   │
│   │               UniverseAgent                         │   │
│   │  (from @universe-agent/agent package)               │   │
│   └─────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
```
...
```typescript
import { UniverseAgentServer } from "@universe-agent/acp";
import { CompositeBackend, FilesystemBackend, StateBackend } from "@universe-agent/agent";
...
UniverseAgent is available in the [ACP Agent Registry](https://agentclientprotocol.com/registry/index) for one-click installation in Zed and JetBrains IDEs. The registry manifest is at `agent.json`:
...
- [UniverseAgent Documentation](https://github.com/langchain-ai/deepagentsjs)
