...
/**
 * Unit tests for the UniverseAgent ACP Server
 */
...
// Mock the @universe-agent/agent module
vi.mock('@universe-agent/agent', () => {
  ...
  return {
    createUniverseAgent: vi.fn().mockReturnValue({
...
describe('UniverseAgentServer', () => {
  let defaultConfig: UniverseAgentConfig;
  let defaultOptions: UniverseAgentServerOptions;
  ...
  describe('constructor', () => {
    it('should create server with single agent config', () => {
      const server = new UniverseAgentServer(defaultOptions);
      expect(server).toBeInstanceOf(UniverseAgentServer);
    });

    it('should create server with multiple agent configs', () => {
      const options: UniverseAgentServerOptions = {
        agents: [
      ...
      const server = new UniverseAgentServer(options);
      expect(server).toBeInstanceOf(UniverseAgentServer);
    });
  ...
  describe('debug logging', () => {
    it('should log when debug is enabled', () => {
      ...
      new UniverseAgentServer(options);
...
describe('UniverseAgentServer handlers', () => {
  ...
  describe('handleInitialize', () => {
    it('should return server capabilities', async () => {
      ...
      const serverAny = server as unknown as {
        handleInitialize: (params: Record<string, unknown>) => Promise<Record<string, unknown>>;
      };
      ...
      expect(result.authMethods.find((m: any) => m.type === 'env_var')).toBeDefined();
      expect(result.authMethods.find((m: any) => m.id === 'universe-agent-setup')).toBeDefined();
    });
  ...
  describe('handleNewSession', () => {
    ...
    it('should throw for unknown agent', async () => {
      ...
      const serverAny = server as unknown as {
        handleNewSession: (params: Record<string, unknown>, conn: unknown) => Promise<unknown>;
      };

      const mockConn = { sessionUpdate: vi.fn().mockResolvedValue(undefined) };

      await expect(
        serverAny.handleNewSession({ configOptions: { agent: 'unknown-agent' } }, mockConn),
...
describe('UniverseAgentServer configuration', () => {
  it('should handle agent with all options', () => {
    const fullConfig: UniverseAgentConfig = {
      name: 'full-agent',
  ...
  it('should handle multiple agents with different configurations', () => {
    const agents: UniverseAgentConfig[] = [
      {
        name: 'coding-agent',
...
describe('UniverseAgentServer streaming', () => {
  ...
  it('should have sendToolCall method', () => {
    ...
    const serverAny = server as unknown as {
      sendToolCall: (sessionId: string, conn: unknown, toolCall: unknown) => Promise<void>;
    };
  ...
  it('should have sendToolCallUpdate method', () => {
    ...
    const serverAny = server as unknown as {
      sendToolCallUpdate: (sessionId: string, conn: unknown, toolCall: unknown) => Promise<void>;
    };
  ...
  it('should have sendPlanUpdate method', () => {
    ...
    const serverAny = server as unknown as {
      sendPlanUpdate: (sessionId: string, conn: unknown, entries: unknown[]) => Promise<void>;
    };
...
describe('Slash Commands', () => {
  ...
  it('should switch to ask mode via /ask command', async () => {
    ...
    const { sessionId } = await serverAny.handleNewSession({}, mockConn);
    await serverAny.handlePrompt({ sessionId, prompt: [{ type: 'text', text: '/ask' }] }, mockConn);
...
describe('Thinking / Reasoning Messages', () => {
  it('should route thinking blocks as thought chunks', async () => {
    ...
    await serverAny.handleAIMessage(session, fakeAIMessage as any, new Map(), mockConn);
    ...
    expect(thoughtCalls[0]![0].update.content.text).toBe('Let me think about this...');
...
describe('Tool Call Enhancements', () => {
  it('should include locations in tool call notifications', async () => {
    ...
    const serverAny = server as unknown as {
      sendToolCall: (sessionId: string, conn: unknown, toolCall: ToolCallInfo) => Promise<void>;
    };
...
describe('Human-in-the-Loop (Permission Requests)', () => {
  ...
  it('should return allow when user selects allow-once', async () => {
    ...
    const result = await serverAny.requestToolPermission(session, mockConn, sampleToolCall);
  ...
  it('should cache allow-always and skip subsequent prompts', async () => {
    ...
    const result1 = await serverAny.requestToolPermission(session, mockConn, sampleToolCall);
    expect(result1).toBe('allow');
    expect(mockConn.requestPermission).toHaveBeenCalledTimes(1);

    const result2 = await serverAny.requestToolPermission(session, mockConn, sampleToolCall);
  ...
  it('should fall back to allow on permission request error', async () => {
    ...
    const mockConn = {
      sessionUpdate: vi.fn().mockResolvedValue(undefined),
      requestPermission: vi.fn().mockRejectedValue(new Error('Connection lost')),
    };
...
describe('Terminal Integration', () => {
  ...
  it('should execute command via terminal and return output', async () => {
    ...
    const result = await serverAny.executeWithTerminal(session, mockConn, toolCall);
  ...
  it('should handle terminal creation failure gracefully', async () => {
    ...
    const mockConn = {
      sessionUpdate: vi.fn().mockResolvedValue(undefined),
      createTerminal: vi.fn().mockRejectedValue(new Error('Terminal not supported')),
    };
  ...
  it('should send in-progress update during execution', async () => {
    ...
    const inProgressUpdate = updateCalls.find((c: any) => c[0]?.update?.status === 'in_progress');
...
describe('Session History Replay', () => {
  it('should send available_commands_update on load', async () => {
    ...
    const serverAny = server as unknown as {
      ...
      handleLoadSession: (params: Record<string, unknown>, conn: unknown) => Promise<unknown>;
    };
