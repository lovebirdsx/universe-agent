/**
 * Integration tests for the UniverseAgent ACP Server CLI
 *
  ...
  async start(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.process.stderr?.on('data', (data: Buffer) => {
        ...
        if (
          lines.some((l) => l.includes('Server started') || l.includes('waiting for connections'))
        ) {
...
describe('CLI Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'universe-agent-cli-test-'));
  ...
  describe('ACP Protocol - Initialize', () => {
    ...
    it('should respond to initialize request', async () => {
      ...
      const agentInfo = response.result?.agentInfo as
        ...
        | undefined;
      expect(agentInfo?.name).toBe('universe-agent-acp');
  ...
  describe('Debug Logging', () => {
    it('should output debug logs to stderr when --debug is set', async () => {
      ...
      const stderr = helper.getStderr();
      expect(stderr.some((line) => line.includes('[universe-agent-acp]'))).toBe(true);
    });
    ...
    it('should log client connection info in debug mode', async () => {
      ...
      const stderr = helper.getStderr();
      expect(
        stderr.some((line) => line.includes('Client connected') || line.includes('my-test-client')),
      ).toBe(true);
    });
  });
...
describe('CLI Help and Version', () => {
  it('should show help with --help flag', async () => {
    ...
    expect(result.stdout).toContain('UniverseAgent ACP Server');
  ...
  it('should show version with --version flag', async () => {
    ...
    expect(result.code).toBe(0);
    expect(result.stdout).toContain('universe-agent-acp');
  });
});
