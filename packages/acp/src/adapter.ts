  ...
  return content.map((block) => {
    ...
    switch (block.type) {
      ...
      case 'resource': {
        ...
        const resource = b.resource as { uri?: string; text?: string } | undefined;
...
export function langChainMessageToACP(message: BaseMessage): ContentBlock[] {
  return langChainContentToACP(message.content as string | Array<{ type: string; text?: string }>);
}
 ...
 */
export function formatToolCallTitle(toolName: string, args: Record<string, unknown>): string {
  switch (toolName) {
    case 'read_file':
