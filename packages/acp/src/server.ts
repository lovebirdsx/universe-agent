/**
 * UniverseAgent ACP Server Implementation
 *
 * This module provides an ACP (Agent Client Protocol) server that wraps
 * UniverseAgent, enabling integration with IDEs like Zed, JetBrains, and others.
 *
...
const DEFAULT_AUTH_METHODS: ACPAuthMethod[] = [
  ...
  {
    id: 'universe-agent-setup',
    name: 'UniverseAgent Setup',
 ...
 *
 * Wraps UniverseAgent with the Agent Client Protocol, enabling communication
...
export class UniverseAgentServer {
  ...
  constructor(options: UniverseAgentServerOptions) {
    this.serverName = options.serverName ?? 'universe-agent-acp';
    ...
    const agentConfigs = Array.isArray(options.agents) ? options.agents : [options.agents];
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      this.connection = new AgentSideConnection((conn) => this.createAgentHandler(conn), stream);
  ...
  private createAgentHandler(conn: AgentSideConnection): Agent {
    return {
      ...
      authenticate: (params) => this.handleAuthenticate(params as AuthenticateRequest),
      newSession: (params) => this.handleNewSession(params as NewSessionRequest, conn),
      loadSession: (params) => this.handleLoadSession(params as LoadSessionRequest, conn),
      prompt: (params) => this.handlePrompt(params as PromptRequest, conn),
      cancel: (params) => this.handleCancel(params as CancelNotification),
      setSessionMode: (params) => this.handleSetSessionMode(params as SetSessionModeRequest),
    };
  }
  ...
  private async handleInitialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const clientName = clientInfo?.name ?? (params.clientName as string) ?? 'unknown';
    const clientVersion = clientInfo?.version ?? (params.clientVersion as string) ?? 'unknown';

    this.log('Client connected:', clientName, clientVersion);

    // Store client capabilities
    const clientCaps = params.clientCapabilities as Record<string, unknown> | undefined;
    if (clientCaps) {
  ...
  private async handleNewSession(
    ...
    const agentName = (configOptions?.agent as string) ?? [...this.agentConfigs.keys()][0];
  ...
  private async handlePrompt(
    ...
    try {
      ...
      const stopReason = await this.streamAgentResponse(session, agent, humanMessage, conn);
  ...
  private async streamAgentResponse(
    ...
    for await (const event of stream) {
      ...
      if (this.currentPromptAbortController?.signal.aborted) {
        this.log('Stream cancelled, cleaning up tool calls:', activeToolCalls.size);
      ...
      for (const message of messages) {
        ...
        this.log('Processing message:', {
          ...
          contentPreview:
            typeof message.content === 'string' ? message.content.slice(0, 100) : '[complex]',
        });

        if (AIMessage.isInstance(message)) {
          await this.handleAIMessage(session, message as AIMessage, activeToolCalls, conn);
        } else if (ToolMessage.isInstance(message)) {
          // Handle tool completion
          await this.handleToolMessage(session, message as ToolMessage, activeToolCalls, conn);
        }
      }
  ...
  private async handleToolMessage(
    ...
    const resultPreview = resultContent.slice(0, 100) + (resultContent.length > 100 ? '...' : '');
  ...
  private async sendToolCall(
    ...
    const locations = extractToolCallLocations(toolCall.name, toolCall.args, this.workspaceRoot);
  ...
  /**
   * Create a UniverseAgent instance for the given configuration
   */
...
/**
 * Create and start a UniverseAgent ACP server
 *
 * Convenience function for quick server setup
 *
 * @example
 * ```typescript
 * import { startServer } from "@universe-agent/acp";
 *
 * await startServer({
 *   agents: {
 *     name: "my-agent",
...
export async function startServer(
  options: UniverseAgentServerOptions,
): Promise<UniverseAgentServer> {
