/**
 * Logger utility for UniverseAgent ACP Server
 *
...
/**
 * Logger class for UniverseAgent ACP Server
 */
export class Logger {
   ...
   */
  private formatMessage(level: LogLevel, args: unknown[], includeTimestamp: boolean): string {
  ...
  private write(level: LogLevel, args: unknown[]): void {
    ...
    if (this.debug || levelNum >= LOG_LEVELS.warn) {
      const stderrMessage = this.formatMessage(level, args, this.timestampsForStderr);
      console.error(stderrMessage);
    }

    // Write to file if configured
    if (this.fileStream) {
      const fileMessage = this.formatMessage(level, args, this.timestampsForFile);
