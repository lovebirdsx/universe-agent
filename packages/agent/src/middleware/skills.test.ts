...
describe('createSkillsMiddleware', () => {
  describe('beforeAgent', () => {
    it('should load skills from configured sources', async () => {
      ...
      expect(result?.skillsMetadata[0].path).toBe('/skills/user/web-research/SKILL.md');
    });
...
# Project Skill`;
      ...
      expect(result).toBeDefined();
      expect(result?.skillsMetadata).toHaveLength(1);
      expect(result?.skillsMetadata[0].description).toBe('Project version of web research');
    });
...
This skill has no valid frontmatter.`;
    ...
    it('should skip if skillsMetadata already in state', async () => {
      ...
      const existingMetadata = [{ name: 'cached', description: 'cached skill' }];
      // @ts-expect-error - typing issue in LangChain
      const result = await middleware.beforeAgent?.({
...
`;
      ...
      expect(result?.skillsMetadata).toEqual([]);
      expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('content too large'));
...
# Invalid Name Skill`;
      ...
      const mockBackend = createMockBackend({
        ...
        directories: {
          '/skills/user/': [{ name: 'Invalid_Name_With_Underscores', type: 'directory' }],
        },
      });
...
# Tools Skill`;
      ...
      expect(result?.skillsMetadata).toHaveLength(1);
      expect(result?.skillsMetadata[0].allowedTools).toEqual(['read_file', 'write_file', 'grep']);
    });
...
# Broken YAML Skill`;
    ...
    it('should handle Windows-style backslash paths', async () => {
      ...
      expect(result?.skillsMetadata).toHaveLength(1);
      expect(result?.skillsMetadata[0].name).toBe('web-research');
      expect(result?.skillsMetadata[0].path).toBe('C:\\skills\\user\\web-research\\SKILL.md');
    });
  });
  ...
  describe('wrapModelCall', () => {
    it('should inject skills into system prompt', () => {
      ...
      expect(modifiedRequest.systemMessage.text).toContain('/skills/user/web-research/SKILL.md');
    });
    ...
    it('should not show allowed tools line if skill has no allowed tools', () => {
      ...
      const allowedToolsCount = (modifiedRequest.systemMessage.text.match(/Allowed tools:/g) || [])
        .length;
  ...
  describe('integration', () => {
    ...
    it('should restore skills from checkpoint and inject into prompt', async () => {
      ...
      expect(modifiedRequest.systemMessage.text).toContain('Restored from checkpoint');
    });
  });
});
...
describe('skillsMetadataReducer', () => {
  ...
  function createSkill(name: string, description = 'A test skill'): SkillMetadataEntry {
    return {
      name,
      description,
  ...
  describe('merging behavior', () => {
    ...
    it('should handle multiple overlapping skills (update wins)', () => {
      ...
      const update = [createSkill('skill-a', 'Updated A'), createSkill('skill-c', 'Updated C')];
    ...
    it('should preserve order: current skills first, then new skills from update', () => {
      ...
      expect(result.map((s) => s.name)).toEqual(['skill-a', 'skill-b', 'skill-c', 'skill-d']);
    });
  });
  ...
  describe('parallel subagent simulation', () => {
    it('should handle concurrent updates from multiple parallel subagents', () => {
      ...
      const afterSubagent1 = skillsMetadataReducer(mainAgentSkills, subagent1Update);
      const afterSubagent2 = skillsMetadataReducer(afterSubagent1, subagent2Update);
...
describe('validateMetadata', () => {
  ...
  it('should coerce non-string values to strings', () => {
    const result = validateMetadata({ count: 42, active: true }, '/skills/s/SKILL.md');
...
describe('formatSkillAnnotations', () => {
  it('should format both license and compatibility', () => {
    ...
    expect(formatSkillAnnotations(skill)).toBe('License: MIT, Compatibility: Python 3.10+');
  });
  ...
  it('should format compatibility only', () => {
    ...
    expect(formatSkillAnnotations(skill)).toBe('Compatibility: Requires poppler');
  });
...
describe('formatSkillsList with annotations', () => {
  it('should include both license and compatibility in annotations', () => {
    ...
    expect(result).toContain('(License: Apache-2.0, Compatibility: Requires poppler)');
  });
    ...
    await agent.invoke(
      {
        ...
        files: {
          ...
          '/skills/project/project-skill/SKILL.md': createFileData(projectSkillMd),
        },
      } as any,
      {
