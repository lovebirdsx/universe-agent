...
describe('read_file character-based truncation', () => {
   ...
   */
  function createFileData(content: string | Uint8Array, mimeType: string = 'text/plain'): FileData {
    return {
      content,
      mimeType,
  ...
  it('should truncate read_file output when it exceeds character limit', async () => {
    ...
    // Invoke the tool
    const result = await readFileTool.invoke({ file_path: '/large.txt' }, { store: undefined });
  ...
  it('should not truncate read_file output when under character limit', async () => {
    ...
    const result = await readFileTool.invoke({ file_path: '/small.txt' }, { store: undefined });
  ...
  it('should not truncate when toolTokenLimitBeforeEvict is null', async () => {
    ...
    const result = await readFileTool.invoke({ file_path: '/huge.txt' }, { store: undefined });
...
describe('read_file multimodal content blocks', () => {
  ...
  it('should return an image content block for .png files', async () => {
    ...
    const result = await readFileTool.invoke({ file_path: '/image.png' }, { store: undefined });
  ...
  it('should return an image content block for .jpg files', async () => {
    ...
    const result = await readFileTool.invoke({ file_path: '/photo.jpg' }, { store: undefined });
  ...
  it('should return an audio content block for .mp3 files', async () => {
    ...
    const result = await readFileTool.invoke({ file_path: '/audio.mp3' }, { store: undefined });
  ...
  it('should return a video content block for .mp4 files', async () => {
    const binaryData = new Uint8Array([0x00, 0x00, 0x00, 0x20, 0x66, 0x74, 0x79, 0x70]);
    ...
    const result = await readFileTool.invoke({ file_path: '/video.mp4' }, { store: undefined });
  ...
  it('should return a file content block for .pdf files', async () => {
    const binaryData = new Uint8Array([0x25, 0x50, 0x44, 0x46, 0x2d, 0x31, 0x2e, 0x34]);
    const files = {
    ...
    const result = await readFileTool.invoke({ file_path: '/document.pdf' }, { store: undefined });
  ...
  it('should return a text content block with line numbers for text files', async () => {
    ...
    const result = await readFileTool.invoke({ file_path: '/notes.txt' }, { store: undefined });
  ...
  it('should return an error for binary files exceeding MAX_BINARY_READ_SIZE_BYTES', async () => {
    ...
    const result = await readFileTool.invoke({ file_path: '/large.png' }, { store: undefined });

    expect(Array.isArray(result)).toBe(true);
    expect(result[0].type).toBe('text');
    expect(result[0].text).toContain('Error');
    expect(result[0].text).toContain('too large');
    expect(result[0].text).toContain(`${MAX_BINARY_READ_SIZE_BYTES / (1024 * 1024)}MB`);
  });
  ...
  it('should return an image block for binary files under MAX_BINARY_READ_SIZE_BYTES', async () => {
    ...
    const result = await readFileTool.invoke({ file_path: '/ok.png' }, { store: undefined });
