...
describe('ASYNC_TASK_SYSTEM_PROMPT', () => {
  ...
  it('should include critical behavioral rules', () => {
    expect(ASYNC_TASK_SYSTEM_PROMPT).toContain('Never auto-check after launching');
...
describe('buildStartTool', () => {
  ...
  it('should return a Command on successful launch', async () => {
    ...
    const { cache, threadsCreate, runsCreate } = createMockClientCache(agentMap);
  ...
  it('should pass the description as a user message to runs.create', async () => {
    ...
    const launchTool = buildStartTool(agentMap, cache, 'Launch a SubAgent');
    await launchTool.invoke({ description: 'analyze the data', agentName: 'researcher' }, config);
  ...
  it('should not include callbackThreadId when runtime has no thread_id', async () => {
    ...
    const launchTool = buildStartTool(agentMap, cache, 'Launch a SubAgent');
    await launchTool.invoke({ description: 'do work', agentName: 'researcher' }, config);
  ...
  it('should use empty string for tool_call_id when config has no toolCall', async () => {
    ...
    expect(result).toBeInstanceOf(Command);
    const cmd = result as Command;
    const messages = (cmd.update as Record<string, unknown>).messages as ToolMessage[];
  ...
  it('should set createdAt to a valid ISO timestamp on the launched job', async () => {
    ...
    const jobs = ((result as Command).update as Record<string, unknown>).asyncTasks as Record<
      string,
      AsyncTask
    >;
...
describe('buildCheckTool', () => {
  ...
  it('should return a Command with running status', async () => {
    ...
    const cmd = result as Command;
    const update = cmd.update as Record<string, unknown>;
    const content = JSON.parse((update.messages as ToolMessage[])[0].content as string);
  ...
  it('should return fallback result when success but no messages', async () => {
    ...
    const content = JSON.parse(
      ((cmd.update as Record<string, unknown>).messages as ToolMessage[])[0].content as string,
    );
  ...
  it('should return an error when runs.get throws', async () => {
    ...
    expect(result).toBeInstanceOf(ToolMessage);
    expect((result as unknown as ToolMessage).content).toContain('Failed to get run status');
    expect((result as unknown as ToolMessage).content).toContain('connection refused');
  });
  ...
  it('should preserve createdAt and updatedAt from the existing job', async () => {
    ...
    const jobs = ((result as Command).update as Record<string, unknown>).asyncTasks as Record<
      ...
      AsyncTask
    >;
    expect(jobs[jobWithTimestamps.taskId].createdAt).toBe('2024-06-01T10:00:00.000Z');
    expect(jobs[jobWithTimestamps.taskId].updatedAt).toBe('2024-06-01T11:00:00.000Z');
  });
});
...
describe('buildUpdateTool', () => {
  ...
  it('should return an error when the SDK throws', async () => {
    ...
    expect(result).toBeInstanceOf(ToolMessage);
    expect((result as unknown as ToolMessage).content).toContain('Failed to update async subagent');
    expect((result as unknown as ToolMessage).content).toContain('server unavailable');
  });
  ...
  it('should preserve createdAt and checkedAt from the existing job', async () => {
    ...
    >;
    expect(jobs[jobWithTimestamps.taskId].createdAt).toBe('2024-06-01T10:00:00.000Z');
    expect(jobs[jobWithTimestamps.taskId].checkedAt).toBe('2024-06-01T10:30:00.000Z');
  });
});
...
describe('buildCancelTool', () => {
  ...
  it('should call runs.cancel with the correct thread and run IDs', async () => {
    ...
    expect(runsCancel).toHaveBeenCalledWith(trackedJob.threadId, trackedJob.runId);
  });
  ...
  it('should return an error when the SDK throws', async () => {
    ...
    expect(result).toBeInstanceOf(ToolMessage);
    expect((result as unknown as ToolMessage).content).toContain('Failed to cancel run');
    expect((result as unknown as ToolMessage).content).toContain('permission denied');
  });
  ...
  it('should preserve createdAt and checkedAt, and set updatedAt to reflect the cancellation', async () => {
    ...
    expect(updatedAt.getTime()).toBeGreaterThanOrEqual(before.getTime());
    expect(updatedAt.getTime()).toBeLessThanOrEqual(after.getTime());
    expect(jobs[jobWithTimestamps.taskId].checkedAt).toBe('2024-06-01T11:30:00.000Z');
  });
});
...
describe('buildListTool', () => {
  ...
  it('should return a string when state has no jobs', async () => {
    ...
    expect(result).toBeInstanceOf(ToolMessage);
    expect((result as unknown as ToolMessage).content).toContain('No async subagent tasks tracked');
  });
...
describe('createAsyncSubAgentMiddleware', () => {
  ...
  it('should inject the agent name and description into the launch tool', () => {
    ...
    const launchTool = (middleware.tools ?? []).find((t) => t.name === 'start_async_task');
  ...
  it('should create independent middleware instances for separate calls', () => {
    ...
    const launchDesc1 = (m1.tools ?? []).find((t) => t.name === 'start_async_task')?.description;
    const launchDesc2 = (m2.tools ?? []).find((t) => t.name === 'start_async_task')?.description;
