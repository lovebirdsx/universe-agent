...
import { createSummarizationMiddleware, type SummarizationEvent } from './summarization.js';
import type { BackendProtocol, FileDownloadResponse, WriteResult } from '../backends/protocol.js';
...
describe('createSummarizationMiddleware', () => {
  ...
  describe('fraction trigger', () => {
    ...
    it('should not trigger when token count is below fraction threshold', async () => {
      ...
      const messages = [new HumanMessage({ content: 'Hello' }), new AIMessage({ content: 'Hi' })];
  ...
  describe('argument truncation', () => {
    it('should truncate large tool call arguments', async () => {
      ...
      if (aiMessage?.tool_calls) {
        ...
        expect(aiMessage.tool_calls[0].args.content.length).toBeLessThan(largeContent.length);
      }
    });
  });
  ...
  describe('multiple triggers', () => {
    it('should support array of triggers', async () => {
      ...
      const messages = Array.from(
        { length: 10 },
        (_, i) => new HumanMessage({ content: `Message ${i} with some content` }),
      );
  ...
  describe('chained summarization', () => {
    it('should compute cutoff index correctly across three chained summarizations', async () => {
      ...
      const { result: result1, capturedRequest: req1 } = await callWrapModelCall(middleware, {
        messages: makeStateMessages(8),
      });

      expect(isCommand(result1)).toBe(true);
      const event1 = (result1 as any).update._summarizationEvent as SummarizationEvent;
      expect(event1.cutoffIndex).toBe(6);
      expect(req1).not.toBeNull();
      // Captured request should have [summary, S6, S7]
      expect(req1!.messages.slice(1).map((m: BaseMessage) => m.content)).toEqual(['S6', 'S7']);
      ...
      const { result: result2, capturedRequest: req2 } = await callWrapModelCall(middleware, {
        messages: makeStateMessages(14),
        _summarizationEvent: event1,
      });

      expect(isCommand(result2)).toBe(true);
      const event2 = (result2 as any).update._summarizationEvent as SummarizationEvent;
      expect(event2.cutoffIndex).toBe(12);
      expect(req2).not.toBeNull();
      expect(req2!.messages.slice(1).map((m: BaseMessage) => m.content)).toEqual(['S12', 'S13']);
      ...
      const { result: result3, capturedRequest: req3 } = await callWrapModelCall(middleware, {
        messages: makeStateMessages(20),
        _summarizationEvent: event2,
      });

      expect(isCommand(result3)).toBe(true);
      const event3 = (result3 as any).update._summarizationEvent as SummarizationEvent;
      expect(event3.cutoffIndex).toBe(18);
      expect(req3).not.toBeNull();
      expect(req3!.messages.slice(1).map((m: BaseMessage) => m.content)).toEqual(['S18', 'S19']);
    });
  });
      ...
      for (const msg of preservedMessages) {
        if (ToolMessage.isInstance(msg)) {
          ...
          const hasMatchingAI = preservedMessages.some(
            (m) => AIMessage.isInstance(m) && m.tool_calls?.some((tc) => tc.id === toolCallId),
          );
  ...
  describe('ContextOverflowError handling', () => {
    it('should catch ContextOverflowError and fall back to summarization', async () => {
      ...
      const { result, capturedRequest } = await callWrapModelCall(middleware, { messages }, () => {
        callCount++;
        if (callCount === 1) {
  ...
  describe('token estimation calibration', () => {
    it('should calibrate multiplier after ContextOverflowError and trigger proactive summarization on subsequent calls', async () => {
      ...
      const { result: result1 } = await callWrapModelCall(middleware, { messages }, () => {
        callCount++;
        if (callCount === 1) {
          throw new ContextOverflowError('prompt is too long');
        }
      ...
      callCount = 0;
      const { result: result2 } = await callWrapModelCall(middleware, { messages }, () => {
        callCount++;
        return new AIMessage({ content: 'OK' });
      });
  ...
  describe('tool result compaction', () => {
    it('should compact tool results when all messages would be summarized', async () => {
      ...
      for (const msg of capturedRequest!.messages) {
        if (ToolMessage.isInstance(msg)) {
          ...
          expect((msg.content as string).length).toBeLessThan(largeContent.length);
      ...
      for (const msg of capturedRequest!.messages) {
        if (ToolMessage.isInstance(msg)) {
          ...
          const matchingAI = capturedRequest!.messages.find(
            (m) =>
              AIMessage.isInstance(m) && m.tool_calls?.some((tc) => tc.id === msg.tool_call_id),
          );
    ...
    it('should not compact when tool results are already small enough', async () => {
      ...
      const messages: BaseMessage[] = [
        ...
        new AIMessage({
          content: '',
          tool_calls: [{ id: 'tc1', name: 'read_file', args: { path: '/a.json' } }],
        }),
        new ToolMessage({
