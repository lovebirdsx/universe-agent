...
describe('StateBackend', () => {
  ...
  it('should handle large tool result interception via middleware', async () => {
    ...
    expect(result.update.files['/large_tool_results/test_123'].content).toBe(largeContent);

    expect(result.update.messages).toHaveLength(1);
    expect(result.update.messages[0].content).toContain('Tool result too large');
    expect(result.update.messages[0].content).toContain('/large_tool_results/test_123');
  });
  ...
  describe('fileFormat: v1', () => {
    ...
    it('should upload files as v1 format', () => {
      ...
      const result = backend.uploadFiles(files);
      expect(result[0].error).toBeNull();
      expect((result as any).filesUpdate['/hello.txt'].content).toEqual(['Hello']);
    });
  });
