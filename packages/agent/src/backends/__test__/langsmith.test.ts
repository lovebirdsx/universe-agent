...
describe('LangSmithSandbox', () => {
  ...
  describe('downloadFiles()', () => {
    ...
    it('maps LangSmithResourceNotFoundError to file_not_found', async () => {
      const sandbox = makeSandbox();
      mockRead.mockRejectedValue(new MockLangSmithResourceNotFoundError('file not found'));
    ...
    it("maps LangSmithSandboxError with 'is a directory' to is_directory", async () => {
      const sandbox = makeSandbox();
      mockRead.mockRejectedValue(new MockLangSmithSandboxError('/tmp/mydir: is a directory'));
    ...
    it('maps generic LangSmithSandboxError to file_not_found', async () => {
      const sandbox = makeSandbox();
      mockRead.mockRejectedValue(new MockLangSmithSandboxError('some other sandbox error'));
  ...
  describe('uploadFiles()', () => {
    ...
    it('maps thrown error to permission_denied', async () => {
      ...
      const [result] = await sandbox.uploadFiles([['/tmp/file.txt', new Uint8Array()]]);
