...
describe('CompositeBackend', () => {
  ...
  it('should route operations between StateBackend and StoreBackend', async () => {
    ...
    expect(result2.matches!.some((m) => m.path === '/memories/readme.md')).toBe(true);

    const globResult = await composite.glob('**/*.md', '/');
    expect(globResult.error).toBeUndefined();
    expect(globResult.files!.some((i) => i.path === '/memories/readme.md')).toBe(true);
  });
  ...
  it('should handle multiple routes', async () => {
    ...
    const resMem = await composite.write('/memories/important.md', 'long-term memory');
    ...
    const resCache = await composite.write('/cache/session.json', 'cached session');
    ...
    const globResult = await composite.glob('**/*.md', '/');
    expect(globResult.error).toBeUndefined();
    expect(globResult.files!.some((i) => i.path === '/memories/important.md')).toBe(true);
  ...
  it('should handle large tool result interception with default route', async () => {
    ...
    expect(result.update.files['/large_tool_results/test_789'].content).toEqual(largeContent);

    expect(result.update.messages).toHaveLength(1);
    expect(result.update.messages[0].content).toContain('Tool result too large');
  });
  ...
  it('should work with FilesystemBackend as default and StoreBackend route', async () => {
    ...
    try {
      ...
      expect(infosMem.files!.some((i) => i.path === '/memories/notes.md')).toBe(true);
      ...
      expect(gm2.matches!.some((m) => m.path === '/memories/notes.md')).toBe(true);
  ...
  it('should work with StoreBackend as default and another StoreBackend route', async () => {
    ...
    const res2 = await composite.write('/memories/important.txt', 'routed store content');
    ...
    expect(grepResult1.matches!.some((m) => m.path === '/notes.txt')).toBe(true);

    const grepResult2 = await composite.grep('routed', '/');
    expect(grepResult2.error).toBeUndefined();
    expect(grepResult2.matches!.some((m) => m.path === '/memories/important.txt')).toBe(true);
  });
  ...
  describe('execute', () => {
    ...
    it('should throw error when default backend is not sandbox', () => {
      ...
      expect(() => composite.execute('echo hello')).toThrow("doesn't support command execution");
    });
