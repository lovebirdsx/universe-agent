      ...
      for (const msg of (data as any).messages ?? []) {
        for (const tc of msg.tool_calls ?? []) {
          if (tc.name === 'task') {
            ...
            console.log(`[lifecycle] PENDING  → subagent "${tc.args?.subagent_type}" (${tc.id})`);
          }
        }
      }
    }
      ...
      for (const msg of (data as any).messages ?? []) {
        if (msg.type === 'tool') {
          ...
          if (subagent) {
            ...
            console.log(`[lifecycle] COMPLETE → subagent "${subagent.type}" (${msg.tool_call_id})`);
            console.log(`  Result preview: ${String(msg.content).slice(0, 120)}...`);
          }
        }
      }
    }
