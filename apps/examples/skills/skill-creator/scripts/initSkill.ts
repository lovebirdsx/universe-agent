...
function validateSkillName(name: string): { valid: boolean; error?: string } {
  ...
  if (!SKILL_NAME_PATTERN.test(name)) {
    return {
      valid: false,
      error: 'Skill name must be lowercase alphanumeric with hyphens only (e.g., web-research)',
    };
  }
  return { valid: true };
}
...
`);
  ...
  fs.writeFileSync(path.join(skillDir, 'scripts', '.gitkeep'), '# Add your scripts here\n');
  fs.writeFileSync(
    path.join(skillDir, 'references', '.gitkeep'),
    '# Add your reference documentation here\n',
  );
  fs.writeFileSync(path.join(skillDir, 'assets', '.gitkeep'), '# Add your assets here\n');
