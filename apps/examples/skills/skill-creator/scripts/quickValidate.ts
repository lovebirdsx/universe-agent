...
function validateSkill(skillDir: string): ValidationResult {
  ...
  if (frontmatter.name) {
    ...
    if (!SKILL_NAME_PATTERN.test(name)) {
      errors.push('name must be lowercase alphanumeric with hyphens only (e.g., web-research)');
    }
    if (name !== dirName) {
  ...
  if (frontmatter.description) {
    ...
    if (description.includes('<') || description.includes('>')) {
      warnings.push('description contains angle brackets - ensure this is intentional');
    }
  }
